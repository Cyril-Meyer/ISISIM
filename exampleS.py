...
def example_2d_fill(export=False):
    ...
    gen_pos = generatorS.gen_click_fill(dense_label_2d, d_step=40, d_margin=20)
    ...
    plt.imshow(utils.combine_image_and_maps(dense_label_2d, pos_click_map, np.zeros(dense_label_2d.shape)))
    if export:
        plt.savefig('media/example_2d_s_fill.png')
    plt.show()
...
def example_3d_fill(export=False):
    ...
    gen_pos = generatorS.gen_click_fill(dense_label_3d, d_step=50, d_margin=15)
    ...
    save_filename = None
    if export:
        save_filename = 'media/example_3d_s_fill.gif'
    utils.show_stack(utils.combine_image_and_maps(dense_label_3d, pos_click_map, np.zeros(dense_label_3d.shape)), save_filename=save_filename)
...
def benchmark_(dense_label_3d):
    ...
    gen_pos_fill = generatorS.gen_click_fill(dense_label_3d)
